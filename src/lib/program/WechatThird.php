...
class WechatThird
{
	...
	{
		$config = config('wx_third_config');
		if (!$config)
			exception('wechat third config error !', PARAM_ERROR);
		...
		self::$component_access_token = self::getComponentAccessToken();

		if (isset($option['app_id']) && $option['app_id']) {
			$this->app_id = $option['app_id'];
			$this->authorizer_access_token = self::getAuthorizerAccessToken($this->app_id);
		}

	}

	/**
	 * 获取第三方授权小程序的auth_access_token
	 *
	 * @param $app_id
	 *
	 * @return mixed
	 ...
	 */
	public static function getAuthorizerAccessToken ($app_id)
	{
		$db_3rd = Config::get('database.wechat_third');
		$qn_db = Db::connect($db_3rd);

		$auth_access_token = $qn_db->table('rb_auth')->where([
			'auth_appid' => ['eq', $app_id],
	...
	public static function getComponentAccessToken ()
	{
		...
		$comp_access_token = $qn_db->table('rb_index')->where([
			'comp_appid' => ['eq', self::$third_app_id],
		])->value('comp_access_token');

		return $comp_access_token;
	}
	...
	{
		$url = self::API_URL_PREFIX . self::API_MODIFY_DOMAIN . $this->authorizer_access_token;

		$r = $this->httpPost($url, $data, true);
	...
	{
		$url = self::API_URL_PREFIX . self::API_SET_WEB_VIEW_DOMAIN . $this->authorizer_access_token;
	...
	{
		$url = self::API_URL_PREFIX . self::API_GET_WXA_SEARCH_STATUS . $this->authorizer_access_token;
	...
	public function changeWxaSearchStatus ($status)
	{
		...
		$url = self::API_URL_PREFIX . self::API_CHANGE_WXA_SEARCH_STATUS . $this->authorizer_access_token;

		$r = $this->httpPost($url, ['status' => $status],true);//1表示不可搜索，0表示可搜索
		log_w(__CLASS__."\t".__FUNCTION__."\tauthorizer_access_token={$this->authorizer_access_token}\tresult={$r}");
	...
	{
		$url = self::API_URL_PREFIX . self::API_MEMBER_AUTH . $this->authorizer_access_token;

		$r = $this->httpPost($url, ['action' => 'get_experiencer'],true);
		log_w(__CLASS__."\t".__FUNCTION__."\tauthorizer_access_token={$this->authorizer_access_token}\tresult={$r}");
	...
	{
		$url = self::API_URL_PREFIX . self::API_BIND_TESTER . $this->authorizer_access_token;

		$r = $this->httpPost($url, ['wechatid' => $wechat_id],true);
		log_w(__CLASS__."\t".__FUNCTION__."\tauthorizer_access_token={$this->authorizer_access_token}\tresult={$r}");
	...
	{
		$url = self::API_URL_PREFIX . self::API_UNBIND_TESTER . $this->authorizer_access_token;
	...
	{
		$url = self::API_URL_PREFIX . self::API_GET_TEMPLATE_DRAFT_LIST . self::$component_access_token;

		$r = $this->httpGet($url);
		log_w(__CLASS__."\t".__FUNCTION__."\tcomponent_access_token=".self::$component_access_token."\tresult={$r}");
	...
	{
		$url = self::API_URL_PREFIX . self::API_GET_TEMPLATE_LIST . self::$component_access_token;
	...
	{
		$url = self::API_URL_PREFIX . self::API_ADD_TO_TEMPLATE . self::$component_access_token;

		$r = $this->httpPost($url, ['draft_id' => $draft_id],true);
		log_w(__CLASS__."\t".__FUNCTION__."\tcomponent_access_token=".self::$component_access_token."\tresult={$r}");
	...
	{
		$url = self::API_URL_PREFIX . self::API_DELETE_TEMPLATE . self::$component_access_token;
		$r = $this->httpPost($url, ['template_id' => $template_id],true);
		if (!$r)
			exception('post request send fail !', ABNORMAL);
		$r_json = json_decode($r, true);
		if (isset($r_json['errcode']) && $r_json['errcode'])
	...
	{
		$url = self::API_URL_PREFIX . self::API_COMMIT . $this->authorizer_access_token;
	...
	{
		$url = self::API_URL_PREFIX . self::API_GET_QRCODE . $this->authorizer_access_token . '&path=' . urlencode($path);
	...
	{
		$url = self::API_URL_PREFIX . self::API_GET_CATEGORY . $this->authorizer_access_token;
	...
	{
		$url = self::API_URL_PREFIX . self::API_GET_PAGE . $this->authorizer_access_token;
	...
	{
		$url = self::API_URL_PREFIX . self::API_SUBMIT_AUDIT . $this->authorizer_access_token;
	...
	public function getAuditStatus($audit_id){
		$url = self::API_URL_PREFIX . self::API_GET_AUDIT_STATUS . $this->authorizer_access_token;

		$r = $this->httpPost($url, ['auditid'=> $audit_id],true);
		log_w(__CLASS__."\t".__FUNCTION__."\tauthorizer_access_token={$this->authorizer_access_token}\tresult={$r}");
	...
	public function getLatestAuditStatus(){
		$url = self::API_URL_PREFIX . self::API_GET_LATEST_AUDIT_STATUS . $this->authorizer_access_token;
	...
	public function release(){
		$url = self::API_URL_PREFIX . self::API_RELEASE . $this->authorizer_access_token;
	...
	public function changeVisitStatus($access_status){
		$url = self::API_URL_PREFIX . self::API_CHANGE_VISIT_STATUS . $this->authorizer_access_token;

		$r = $this->httpPost($url,['action'=>$access_status],true);
		log_w(__CLASS__."\t".__FUNCTION__."\tauthorizer_access_token={$this->authorizer_access_token}\tresult={$r}");
	...
	public function revertCodeRelease(){
		$url = self::API_URL_PREFIX . self::API_REVERT_CODE_RELEASE . $this->authorizer_access_token;
	...
	public function undoCodeAudit(){
		$url = self::API_URL_PREFIX . self::API_UNDO_CODE_AUDIT . $this->authorizer_access_token;
	...
	public function bindmember($wechatid){
		$url = self::API_URL_PREFIX . self::API_MEMBER_BINDTEST . $this->authorizer_access_token;
		$r = $this->httpPost($url, ['wechatid'=> $wechatid],true);
        log_w(__CLASS__."\t".__FUNCTION__."\tauthorizer_access_token={$this->authorizer_access_token}\tresult={$r}");
	...
	public function unbindmember($user_str){
		$url = self::API_URL_PREFIX . self::API_MEMBER_UNBINDTEST . $this->authorizer_access_token;
		$r = $this->httpPost($url, ['userstr'=> $user_str],true);
        log_w(__CLASS__."\t".__FUNCTION__."\tauthorizer_access_token={$this->authorizer_access_token}\tresult={$r}");
	...
	public function unbindmember_null($userstr){
		$url = self::API_URL_PREFIX . self::API_MEMBER_UNBINDTEST . $this->authorizer_access_token;
		$r = $this->httpPost($url, ['userstr'=> $userstr],true);
        log_w(__CLASS__."\t".__FUNCTION__."\tauthorizer_access_token={$this->authorizer_access_token}\tresult={$r}");
	...
	public function memberauthlist(){
		$url = self::API_URL_PREFIX . self::API_MEMBER_LISTMEMBER . $this->authorizer_access_token;
		$r = $this->httpPost($url, ['action'=> 'get_experiencer'],true);
        log_w(__CLASS__."\t".__FUNCTION__."\tauthorizer_access_token={$this->authorizer_access_token}\tresult={$r}");
    ...
    public function createOpen ($auth_appid)
    {
        ...
        $url = self::API_URL_PREFIX . self::API_OPEN_CREATE . $this->authorizer_access_token;

        $r = $this->httpPost($url, ['appid' => $auth_appid], true);
        log_w(__CLASS__ . "\t" . __FUNCTION__ . "\tauthorizer_access_token={$this->authorizer_access_token}\tresult={$r}");
    ...
    public function bindOpen ($auth_appid, $open_appid)
    {
        ...
        $r = $this->httpPost($url, ['appid' => $auth_appid, 'open_appid' => $open_appid], true);
        log_w(__CLASS__ . "\t" . __FUNCTION__ . "\tauthorizer_access_token={$this->authorizer_access_token}\tresult={$r}");
